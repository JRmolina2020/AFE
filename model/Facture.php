...
class Facture
{
    ...
    {
        $sql = "SELECT
        CO.ID as ID,CO.CONSECUTIVO as CONSECUTIVO,CO.NOFACTURA as NOFACTURA,CO.FEC_COMPRA as FECHA,
        ...
        INNER JOIN clientes C
        ON V.TERCERO = C.CODIGO
        WHERE DATE(CO.FEC_COMPRA)=curdate() and CO.TOTAL >0 and CO.IDTIPO BETWEEN 3 and 6";
        return ejecutarConsulta($sql);
    }
    ...
    public function notacreditounica($numeronota)
    {
        ...
        INNER JOIN ciudades CI
        ON CI.CODIGO = C.CIUDAD
        WHERE CO.CONSECUTIVO  = '$numeronota' and CO.TOTAL >0 and CO.IDTIPO BETWEEN 3 and 6";
        return ejecutarConsulta($sql);
    }
    //DETALLE VENTA
